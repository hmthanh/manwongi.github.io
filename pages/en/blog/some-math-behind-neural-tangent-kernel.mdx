...
\mathbb{E}[\big(w^{ (l) }_{im} \sigma ( \tilde {A}^{ (l) }_i(\mathbf{x}))\big)^2] &= \mathbb{E}[{w^{ (l) }_{im}}^2]\mathbb{E}[ \sigma ( \tilde {A}^{ (l) }_i(\mathbf{x}))^2] =  \sigma _w^2  \sigma ^{ (l) }(\mathbf{x}, \mathbf{x}) =  \sigma ^{ (l) }(\mathbf{x}, \mathbf{x}) \end{aligned}
$$
...
=& \frac{1}{n_l} \Big[
    ...
    \color{red}{
      { \mathbf{w}^{  (l)  } }^\top
      \begin{bmatrix}
        \dot{\sigma}\left(\tilde{A}_1^{(l)}\right) \dot{\sigma}\left(\tilde{A}_1^{(l)}\right) \sum_{p=1}^{\bar{P}} \frac{\partial \bar{A}_1^{(l)}}{\partial \bar{\theta}_p} \frac{\partial \bar{A}_1^{(l)}}{\partial \bar{\theta}_p}
           & \dots &
          \dot{\sigma}\left(\tilde{A}_1^{(l)}\right) \dot{\sigma}\left(\tilde{A}_{n_l}^{(l)}\right) \sum_{p=1}^{\bar{P}} \frac{\partial \bar{A}_1^{(l)}}{\partial \bar{\theta}_p} \frac{\partial \bar{A}_{n l}^{(l)}}{\partial \bar{\theta}_p} \\
        \vdots \\
        \dot{\sigma}\left(\tilde{A}_{n_l}^{(l)}\right) \dot{\sigma}\left(\tilde{A}_1^{(l)}\right) \sum_{p=1}^{\tilde{P}} \frac{\partial \tilde{A}_{n_l}^{(l)}}{\partial \tilde{\theta}_p} \frac{\partial \tilde{A}_1^{(l)}}{\partial \tilde{\theta}_p}
          & \dots &
          \dot{\sigma}\left(\tilde{A}_{n_l}^{(l)}\right) \dot{\sigma}\left(\tilde{A}_{n_l}^{(l)}\right) \sum_{p=1}^{\tilde{P}} \frac{\partial \tilde{A}_{n_l}^{(l)}}{\partial \tilde{\theta}_p} \frac{\partial \tilde{A}_1^{(l)}}{\partial \tilde{\theta}_p}
      \end{bmatrix}
      \mathbf{w}^{  (l)  }
    }
\color{black}{\Big]} \\
